...
import { createAnimePaths, getImageUrl } from '@/utils';
import { MEDIA_QUERY } from '@/variables';

import { BookmarkIcon, Breadcrumbs, LongArrowIcon, PlayCircleIcon } from '@/components';
...
import {
  ...
  StaffTab,
  VideosTab,
} from '@/components/Features/FullAnimeTabs';
...
// const tabs = ['details', 'episodes', 'characters', 'pictures', 'videos', 'news', 'recommendations'];
...
const AnimePage = React.memo(() => {
  const location = useLocation();
  const navigate = useNavigate();
  const id = Number(useParams<{ id: string }>().id);
  const basePath = createAnimePaths(id).fullAnime;
  const activeTabFromUrl = location.pathname.split(basePath)[1].split('/').filter(Boolean)[0];

  const { item, status } = useAppSelector((state) => state.animeFullById);
  useAbortableDispatch(fetchFullAnimeById, id, !item || item.mal_id !== id);

  const { isLoading, isSuccess, isError } = useFetchStatus(status.item);
  const { src, onLoad, isFallback, resetSrc } = useYoutubeTrailerImage(item && item.trailer.images);
  ...
  //   { name: 'isTablet', maxWidth: 1024 },
  //   { name: 'isMobile', maxWidth: 768 },
  // ]);
  ...
  const tabs = React.useMemo(() => {
    ...
    return [
      ...
      {
        value: 'episodes',
        element: <EpisodesTab id={id} />,
      },
      {
        value: 'characters',
        element: <CharactersTab id={id} />,
      },
      {
        value: 'pictures',
        element: <PicturesTab id={id} />,
      },
      {
        value: 'videos',
        element: <VideosTab id={id} />,
      },
      {
        value: 'news',
        element: <NewsTab id={id} />,
      },
      {
        value: 'recommendations',
        element: <RecommendationsTab id={id} />,
      },
    ];
  }, [item]);
  ...
  if ((isSuccess && !item) || isError) return <NotFound />;
      ...
      <div className="anime__intro anime-intro">
        ...
        <div className="anime-intro__container container">
          ...
          <div className="anime-intro__body">
            <h2 className="anime-intro__title title title--fz-48">{item ? item.title : <Skeleton />}</h2>
            {!item &&
              Array.from({ length: 2 }).map((_, i) => (
