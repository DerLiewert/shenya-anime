  ...
  for (const [key, value] of params.entries()) {
    switch (key) {
      case 'order_by':
        ...
        break;
      case 'type':
        if (animeSearchType.includes(value as AnimeSearchType)) {
          result.type = value as AnimeSearchType;
        }
        break;
      case 'status':
...
//         },
//       ]}
//       fetchItems={fetchAnimeGenres as any}
//       selectItemsState={(state) => state.animeCatalog}
//       fetchGenres={fetchAnimeGenres}
//       selectGenresState={(state) => state.animeGenres}
//       renderItem={(item) => (
//         <AnimeCard key={item.mal_id} item={item} className="catalog-cards__card" />
//       )}
//     />
